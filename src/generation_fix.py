#!/usr/bin/env python3
"""
Fix NOVA-SLM Generation - No More Repetition!
==============================================
"""
...
def generate_fixed(
    ...
    """
    Fixed generation with anti-repetition.
    
    Key improvements:
    - Repetition penalty
    - No repeat n-grams
    - Better sampling (top-p + top-k)
    - Length control
    """
    model.eval()
    ...
    for step in range(max_new_tokens):
        ...
        # 2. NO REPEAT N-GRAMS - ban repeating phrases
        if no_repeat_ngram_size > 0 and generated.shape[1] >= no_repeat_ngram_size:
            for i in range(batch_size):
                ...
                # Find all ngrams with this prefix in history
        ...
        # 3. MIN LENGTH - don't stop too early
        if step < min_length:
